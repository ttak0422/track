...
Status: Superseded. Journal note ids remain `yyyyMMdd`, but current storage is under `journal/` (`<vault>/journal/<yyyyMMdd>.md`) so paths can be derived from file kind plus id and do not need to be cached.

...
A note's title is also a link keyword: `[[title]]` resolves against titles and aliases by exact match, and ambiguous terms fall back to the first note by id (`store.ResolveTerm` uses `LIMIT 1`). If two notes share a title, every `[[title]]` silently points at only one of them and the other becomes unreachable by name.

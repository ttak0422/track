...
- `track toggle (--id N | --title X | --path P) --line N`: two-state shorthand over `task set` — flip between the first open and first done-family state. `--state check|uncheck` forces a result idempotently.

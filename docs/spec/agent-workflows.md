...
- `track meta ... --edit (FILE|-)`: apply a full metadata document — the `doc` YAML with keys `tags`, `description`, `image`, `props` — from a file or stdin, replacing all four as one validated, atomic write (a rejected document changes nothing). The title is not part of the document; use `track rename`. `--edit` cannot be combined with the field flags. This is the round-trip the editor frontends use: read `doc`, edit it, pipe it back.

...
- `track task add (--id N | --title X | --path P) --text S [--priority A] [--sched YYYY-MM-DD] [--due YYYY-MM-DD]`: append a new open task line to the note's end, in the documented token order (text, priority, dates) — the machine's way to add a task without hand-writing a checkbox line. The returned `line` is where the task landed, for follow-up `task set`/`date` calls.
- `track tasks [--id N | --title X | --path P] [--state A,B] [--priority A,B] [--text S] [--due YYYY-MM-DD] [--overdue] [--sort priority]`: list indexed tasks as JSON. Task lines carry optional `[#A]` priority and `[sched:YYYY-MM-DD]`/`[due:YYYY-MM-DD]` date tokens; `--due` keeps open tasks due on or before the date, `--overdue` keeps open tasks past their deadline, `--priority` keeps tasks with one of the listed priority letters (case-insensitive, unprioritized tasks never match), `--text` keeps tasks whose text contains the substring (case-insensitive, literal — `%` is not a wildcard), `--sort priority` orders open tasks first by priority then deadline.
...
- `track meta (--id N | --title X | --path P) [--description S] [--image assets/F] [--icon S] [--set key=value ...] [--unset key ...] [--edit (FILE|-)]`: print a note's metadata (including its typed properties under `props` and its editable YAML document under `doc`), or set it: the page summary (published as `og:description`), the cover image (`og:image`; must be an existing vault asset), the per-note icon (a single-line emoji shown beside the title; overrides the config tag/kind mapping), and typed key-value properties (`--set`/`--unset`; a comma-separated value makes a list). An explicitly empty description/image/icon clears that field. Property values are typed from their text — boolean, number, `YYYY-MM-DD` date, `[[link]]`, else string — and validated against the optional `properties:` schema in the vault config (`<vault>/.track/config.yml`); schema violations also surface in `track doctor`.
...
- `track agenda [--date YYYY-MM-DD]`: list what a calendar day holds — the `notes` created or updated on that day (default today, for "what was worked on that day" lookups; activity days are recorded per note in the sidecar and cover both CLI mutations and direct editor edits), plus the `tasks` open on it: not-done tasks scheduled for or due on the date, due-first. Task tokens are always ISO dates, so a vault configured for another display format is still matched correctly.

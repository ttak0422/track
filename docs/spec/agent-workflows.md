...
Normal command results are one compact JSON object on stdout, and failures print `{"error":"..."}` and exit 1. This covers every dispatched subcommand, including `dump`, whose placeholder result is JSON even though it is emitted directly by the router. The output exceptions are deliberate: `version` prints `track <version>`; `web` prints its listening URL to stderr and then serves until stopped, without a successful JSON result; `export` prints rendered Markdown to stdout unless `--out` is supplied, in which case it prints a JSON path result; and `gen peek` prints the selected Markdown snapshot to stdout. A `--help` request prints prose usage to stdout and exits 0, while a missing or unknown command prints usage to stderr; a real argument-parse failure still has the JSON error result. Agents should parse JSON on the paths that promise it instead of scraping human text.
...
Addressing a vault that is not registered — a checkout's own, most often — takes no setup:

- **A command that names a file needs nothing.** A note sits directly under `<vault>/note/` or `<vault>/journal/`, so `--path` names its vault: commands taking `--path` (`append`, `update`, `meta`, `task set`, `toggle`, `nav`, `export`, `babel`) operate on the vault that path belongs to, whichever vault is active. An explicit `--vault` still wins.
- **Everything else takes `TRACK_VAULT=<path>` as a prefix.** `TRACK_VAULT=<path> track search --query x` reads that vault for one command. A scratch vault needs the directory to exist first — an empty one is accepted, so `TRACK_VAULT="$(mktemp -d)"` works with no `track init`. Commands that name no file (`search`, `new --title`, `notes`, `query`) never infer a vault, so **forgetting the prefix on a write silently uses the default vault** — prefix every call in a script rather than relying on inheritance.
...
- `track vault current`: `{"name","path","source"}` for the active vault (name resolves through the registry even when the vault was selected via `TRACK_VAULT`). `source` says what selected it, in the precedence order above: `flag` (`--vault NAME`), `env` (`TRACK_VAULT`), `default_vault` (the machine config's registry name), `vault_dir` (a registry-less machine config), or `default` (`$HOME/track`). Check it before blaming the config: a `TRACK_VAULT` exported once in a shell profile makes `default_vault` inert for every later command, and the name and path alone read as an ordinary configured vault.
...
Cross-vault references (ADR 0053): `[[vault:title]]` links a note in a registered vault — explicit prefix only, no implicit fallback, and the prefix counts as a qualifier only when it is a registered vault name *other than the vault the note is in* (any other colon-containing key, including the vault's own name, stays an ordinary local title; `track doctor` reports local titles a registration shadows as `shadowed_title`). Anchors and aliases compose as usual (`[[vault:title#Heading|alias]]`). `track resolve --term "vault:title"` resolves in the named vault and adds `"vault"` to its response. `track backlinks` adds `"external"` (notes in other vaults referencing this one, each with its `vault`) and `"unavailable"` (vaults that could not be consulted) when a registry exists. Cross-vault edges are stored as (vault, title) strings, so renaming a note breaks inbound qualified references from other vaults — expect and repair those via the unresolved-link diagnostics.

## Title Model

The sidecar `metadata.title` is authoritative. A note body's H1 headings are ordinary Markdown content. Creating a note writes the title to the sidecar, and renaming a note must use `track rename` or LSP rename so backlinks are rewritten with it.
...
- `track task cycle (--id N | --title X | --path P) --line N [--expect NAME]`: advance the task checkbox on one line to the next state in the state-set order, wrapping at the end — the single-key loop frontends bind. Same write path as `task set`. It asserts the state it read before advancing, so a concurrent edit fails the command instead of advancing from a state it never saw; `--expect` overrides that assertion.
...
- `track mv (--id N | --title X | --path P) --to VAULT [--unlink | --qualify]`: move a whole note
  into a registered vault, preserving its id and sidecar (including its published `slug`). Before
  writing, it checks each local `[[title]]` against the destination. A link that would break refuses
  the move by default; `--unlink` replaces it with its visible text, while `--qualify` rewrites it as
  `[[source-vault:title]]` and therefore requires the source vault to be registered. Referenced body
  assets and the cover image are copied alongside the note; missing assets and same-name/different-
  content collisions fail during preflight. Backlinks left in the source vault become
  `[[VAULT:title]]`, the source body and sidecar go to its trash, and both indexes rebuild. Once any
  destination write has begun, a later failure leaves the source in place, preferring a recoverable
  duplicate to lost text.
- `track meta (--id N | --title X | --path P) [--description S] [--image assets/F] [--icon S] [--flag F] [--unflag F] [--set key=value ...] [--unset key ...] [--edit (FILE|-)]`: print a note's metadata (including its typed properties under `props`, its flags under `flags`, and its editable YAML document under `doc`), or set it: the page summary (published as `og:description`), the cover image (`og:image`; must be an existing vault asset), the per-note icon (a single-line emoji shown beside the title; overrides the config tag/kind mapping), author-assigned flags (`--flag`/`--unflag`, repeatable; the implementation-defined closed set `DEPRECATED`/`CONFIDENTIAL`, ADR 0074 — an unknown value is a hard error), and typed key-value properties (`--set`/`--unset`; a comma-separated value makes a list). An explicitly empty description/image/icon clears that field. Property values are typed from their text — boolean, number, `YYYY-MM-DD` date, `[[link]]`, else string — and validated against the optional `properties:` schema in the vault config (`<vault>/.track/config.yml`); schema violations also surface in `track doctor`.
- `track meta ... --edit (FILE|-)`: apply a full metadata document — the `doc` YAML with keys `title`, `tags`, `description`, `image`, `icon`, `flags`, `props` — from a file or stdin, validated as a whole before anything is written (a rejected document changes nothing). The document is whole-state: omitting `icon` (or any non-title key) clears the stored value, so always start from the current `doc`. A changed title is a rename with `track rename` semantics: uniqueness against the index, backlink rewrite; an empty `title:` leaves the title unchanged. Empty fields render as bare `key:` lines (both bare and `[]`/`{}` forms parse). `--edit` cannot be combined with the field flags. This is the round-trip the editor frontends use: read `doc`, edit it, pipe it back.
- `track query '<expr>'` (or `--saved <name>` for a query named under `queries:` in the vault config): run a table query over notes and print `{"columns":[...],"rows":[{"note_id","title","cells"}],"count"}`. The expression is `TABLE <cols> [FROM #tag] [WHERE <cond> AND ...] [SORT key [DESC]] [LIMIT n]`; a key is either a note attribute (bare `title` or `tags`) or a user property written `props.<key>` (the only way to reach a property) — an unknown bare key is a loud error, not an empty column; conditions are `#tag` filters, typed comparisons (`=`, `!=`, `<`, `>`), or a bare key as a presence check. Tags are hierarchical everywhere (`#a` matches `#a/b`, not `#ab`).
...
- `track agenda [--date YYYY-MM-DD] [--mode activity|planning]`: list a calendar day (default today) in one of two mutually exclusive views. The default `activity` view lists the notes created or updated on the day, for "what was worked on that day" lookups — activity days are recorded per note in the sidecar and cover both CLI mutations and direct editor edits. The `planning` view lists the open dated tasks a working session on the day should pick up — overdue (deadline before the day), scheduled on the day itself, or due within the 7-day planning horizon — in urgency order (nearest date first, then priority `[#A]` > `[#B]` > `[#C]` > none), as `{"date", "mode":"planning", "tasks":[...]}`.
...
the per-vault pages merge in Go on the rank their own SQL assigned (ADR 0062), so no attach limit
bounds how many vaults one search covers. The merged ranking labels each hit with `"vault"` (`""`
for the unregistered active vault) — the vault half of the (vault, id) identity, since a bare id
can name different notes in different vaults. Target a hit's vault in follow-up commands with
`--vault <name>`. The response adds `"unavailable": [{"name","path","error"}]` for vaults that
could not be searched (unmounted, permission denied) instead of failing the whole search.
`--vault NAME` scopes search back to one vault with the single-vault output shape.
...
Monitor metrics on adopted specs (ADR 0076, dashboard reader ADR 0077) — OpenMetrics ingest, Grafana-subset
dashboards, Prometheus-subset alert rules. Expressions are a metric name plus `=` matchers only;
datasources are `{"type":"track","uid":"<data/ filename>"}`. track never fetches: pipe an
endpoint's body in, per ADR 0021:

```sh
curl -s web1:9100/metrics | track metrics scrape --from - --out data/sys.jsonl
...
`track doctor` is a read-only health check that never changes files. It reports vault/sidecar divergence — the kind a cloud sync (e.g. OneDrive) can introduce — as a JSON `issues` array, with `ok: true` when clean. Issue kinds: `missing_sidecar`, `orphan_sidecar`, `stray_file` (e.g. a conflict copy that breaks the `<id>.md` naming rule), `stray_sidecar` (the same copy made of a `.track/notes/<id>.yaml` sidecar), `unreadable_sidecar`, `duplicate_title`, `property_violation` (a note property breaks the configured schema), and `shadowed_title` (a title is captured by a registered vault qualifier). Finding issues is not an error, so it still exits 0; only real failures use the `{"error":...}`/exit 1 contract. Run it before a `reindex --full` if you suspect a partially synced vault, so an orphan sidecar is not mistaken for a delete.

`track doctor --fix` repairs only the file divergences whose intended restore is unambiguous: it writes a fresh `Untitled N` sidecar for a `missing_sidecar`, recreates an empty markdown for an `orphan_sidecar`, keeps the lowest id and renumbers the rest for a `duplicate_title`, and imports a `stray_file` as a new note with a fresh id and title. It reports an `unreadable_sidecar` under `skipped` rather than guessing at its contents, and likewise skips a `stray_sidecar` because choosing between conflicted metadata files is the user's call. `property_violation` and `shadowed_title` are also reported under `skipped`: the intended property value, note rename, or vault rename is a human decision. The response carries `changed`, `fixed`, and `skipped`.

`track fmt` is the style counterpart to `doctor` (which finds breakage): it rewrites files to canonical form in place and returns `{"checked":N,"changed":[...]}`. `track fmt --all` covers every note and journal body plus the sidecar metadata documents under `.track/notes/`, normalizing heading markers (a closed ATX heading `## Title ##` becomes `## Title`), blank lines around headings, trailing whitespace, list bullets, and the top-level key order of a sidecar (the canonical order of the `version, title, tags, created, days, description, image, props, icon, seen_at, read_at, flags, blocks, task_log, slug` document). Values, quoting, and comments are preserved; fenced code blocks are never touched. `track fmt --check` writes nothing and exits non-zero when any file would change — the CI/save-hook mode. Passing explicit paths formats those files (and directories, expanded to markdown) instead of the whole vault.

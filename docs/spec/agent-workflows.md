...
A pull request is public: its title, body, and code comments are visible to anyone
who can read the repository. Vault notes are not public, so never mention a note's
content, title, or id in any of them. Ground a change only in what is already
public — the help vault (`docs/help`), ADRs, and specs — so reviewers can verify it

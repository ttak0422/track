...
- `textDocument/rename` on a `[[link]]` (or on the current note's H1 title line) renames the target note: it rewrites that note's first H1 to the new name and every `[[oldTitle]]` backlink — including the key of `[[oldTitle|display]]` and `[[oldTitle##anchor]]` — in a single workspace edit. Renaming to the same name, or on an unresolved link, is a no-op. The title-rename history (`.track/renames.yaml`) is recorded by the normal save/reindex path, not by the rename request itself.

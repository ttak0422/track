...
1. A configured default wins: `default_template` (notes) or `journal_template` (journals) in the vault config (`<vault>/.track/config.yml`), overridable for one-off runs by `TRACK_DEFAULT_TEMPLATE` / `TRACK_JOURNAL_TEMPLATE`. The value is a template name or a vault path, same as `--template`.

...
- It resolves the vault from the user config file or `setup({ vault_dir = ... })`, defaulting to `$HOME/track` when neither is set (ADR 0015); `TRACK_VAULT` is a test/one-off override.

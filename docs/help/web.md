...
- A metadata editor (the Meta dialog in the note actions menu): the note's title, tags,
  description, cover image, and [[Properties]] as one YAML document, validated and applied by the
  engine — the same round-trip as `track meta --edit` and the Neovim `:Track meta` popup. Changing
  the title renames the note and rewrites its backlinks. The published static site has no editor.

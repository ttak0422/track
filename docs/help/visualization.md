...
- [[Embeds]] — rich media from a standalone `![alt](src)` or a fenced block: YouTube players, Twitter/X
  posts, PDF decks, image URLs, Open Graph link cards, and full Mermaid diagrams.

Both are driven by ordinary Markdown — a fenced block or an image link on its own line — so a note stays

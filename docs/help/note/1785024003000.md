...
`source save` returns a record with a `version` hash. Reuse the same note ID when
retrieving that document again: identical content reuses the saved version, and
changed content creates another. `--original /path/to/file.pdf` also preserves the
exact original file. `--at` records retrieval time, not an inferred publication time.
...
An identical input and recipe on the same note reuses the first result. Changed
settings produce a separate version. Use `--run regeneration-1` for an explicit
regeneration, reusing that key for retries. These commands freeze existing notes;
they do not fetch documents or generate summaries.
...
are not interpreted. `track-fetch-pdf --file document.pdf --note --out document.txt`
extracts these boundaries using Poppler. Check its success before creating a note
from `document.txt`, then preserve it with `source save --original document.pdf`.
Empty pages keep their physical positions; a final page terminator does not add a page. No selector returns the full body. Without `--version`, the

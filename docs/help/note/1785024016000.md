...
- [[Diagrams]] — Mermaid (flowcharts, sequence, class, state, Gantt, and every other type it knows),
  Graphviz DOT graphs, D2 diagrams, and draw.io files rendered with draw.io's own viewer, written
  inline or kept as a `.mmd` / `.dot` / `.d2` / `.drawio` attachment.
...
- [[Metrics dashboards]] — numeric cards, time-series charts, and tables in one grid, with shared filters in live Web and Native readers.
...
- [[Maps]] — a `map` fence with center coordinates, zoom, tile type, and wiki-linked markers,
  drawn as a slippy map.

All are driven by ordinary Markdown — a fenced block or an image link on its own line — so a note stays
plain text and portable. Most visuals also render in the static export ([[CLI]] `export-site`).
Metrics dashboards publish build-time snapshots of their panels; shared filters and refresh require a live reader.

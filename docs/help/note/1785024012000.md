...
- **Columns and keys** come from two separate namespaces. A **bare** identifier is a note attribute:
  `title` (rendered as a link to the note), `tags`, and `body` (see below). A **user
  [[Properties|property]]** (a sidecar value or an inline field) is written `props.<key>` —
  `props.status`, `props.rating` — and that is the *only* way to reach one. Keeping properties under
  `props.` means a bare word never silently picks up a property, and new note attributes can be added
  later without ever colliding with your property names. A multi-valued property fills its cell with
  `a, b`; a `props.<key>` column shows just `<key>` in the table header.
...
- **`WHERE`** conditions are `AND`-combined. Each one is a `#tag` filter, a comparison
  ...
  `owner = "Ada Lovelace"`.
- **`body` matches note bodies by full text.** `body = "text"` keeps notes whose body satisfies the
  expression; `body != "text"` keeps the rest. Inside the value, terms join with `AND` (implicit)
  and an uppercase `OR` separates alternatives — the same grammar as full-text search:
  `body = "quick fox OR turtle"` means `(quick AND fox) OR turtle`. The live workspace resolves it
  through the same SQLite FTS5 index body search uses, so it stays fast on a large vault; on a
  published site the search runs over the published pages. `body` is not a column or a sort key —
  it is a `WHERE`-only filter, so `TABLE body` is a loud error.
...
- **`:layout list`** — a compact one-row-per-note list. The title remains a link, but the redundant
  `title` table header is omitted; this is useful when the query has only `TABLE title`.
- **`:layout board`** — a kanban-style board: one lane per value of the grouping column, cards in
  the query's order.
- **`:layout gallery`** — a grid of cards showing each note's cover image (the `image` in its
  sidecar). A note without one shows its icon as the card face; with no icon either, track's
...
`:by status`; it defaults to the first non-title column. Every layout is read-only — cards, list
entries, and day entries link to their notes, like a table's title cells. If you know Obsidian's
Bases views or org-mode's agenda, this is the same shape of idea.

Card titles are visible by default. Board and gallery blocks may opt into a title-free card when the
other card content already identifies the note:

````markdown
```track-query :layout gallery :title hide
TABLE title, props.status FROM #project
```
````

`:title show` is the default. `:title hide` only changes the visual title in board/gallery cards;
the card link remains available. Lists and calendars always show their title link because it is the
primary visible identity of each entry.
...
Several pages of this site carry a `section` property in their sidecar props; the board lanes them
by it. Because `section` is a user property, it is written
...
Saved queries come from the vault config, so a `saved:` fence expands on a published site too — this
site's own vault config is what would name one.
...
Tags come from note metadata (`track new --tag`, `track append --tag`, the sidecar's `tags`) and
from the body — a hierarchical `#a/b/c` written in prose is indexed too, matching `FROM #a` and
`WHERE #a` exactly as if it were a metadata tag. Only prose counts: `#tags` inside fenced or inline
code stay examples, not data, and a bare `#2026`, a Markdown heading, or a `foo#bar` fragment is
never a tag.

...
the same search in the browser over the bodies its bundle carries: the same
grammar, the same `#tag` filter, the same substring matching, the same snippet.
What it cannot do is rank by relevance — that lives in the index — so its
full-text group is ordered most recently updated first, which is what the search
above falls back to whenever a query cannot use the index either. The bodies are
fetched the first time you search, not when the page opens.

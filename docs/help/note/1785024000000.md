...
lives in. All destinations are validated before writing; track metadata and managed note files are
protected even through symlinks. Different paths resolving to one output are refused. `:tangle yes` (Org's derived file naming) is not supported; name the file explicitly.

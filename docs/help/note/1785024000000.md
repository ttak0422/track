...
The resulting `scripts/build.sh` contains:
...
Targets resolve relative to the note's directory; missing parent directories are created.
All destinations are validated before writing, including symlink resolution. Paths outside the vault,
track metadata, and managed note files are refused. Aliases to one output count as the same target.
A planned file cannot also be a directory for another output. `:tangle yes` is not supported;
name the file explicitly. Omitted `:tangle` and `:tangle no` produce no file. Tangling never runs
blocks, even when noweb references are expanded.
...
It generates files in the vault; home-directory installation and Nix integration remain separate.

...
- **The metadata editor** edits the note's whole editable metadata: title, tags, description, cover
  image, icon, flags, and props. The [[Web workspace]] Meta dialog gives each built-in field a dedicated
  control — a title box (a rename on change), a tags box, a description box, a cover image you can
  upload straight from the browser into the vault assets, an icon box, and checkboxes for the note
  flags — and keeps props as the one free-form YAML block.
...
track meta --title "My note" --flag DEPRECATED
track meta --title "My note"          # prints metadata incl. props, flags, and the editable doc (JSON)
...
Flags are a second kind of note-level marker, distinct from properties. A property is a typed
key-value fact (`status: draft`); a flag is a whole-note judgment from a small, closed set — today
`DEPRECATED` and `CONFIDENTIAL` — that renders as a red stamp over the note itself and as a badge
beside it in lists. `DEPRECATED` also pushes the note down in search results, so an old note stops
crowding out its replacement.

Flags live in the sidecar beside tags, and `track meta` edits them the same way it edits properties:

```sh
track meta --title "Old design doc" --flag DEPRECATED
```

The note now carries the flag, visible in `track meta` output and in its sidecar:

```json
{"title":"Old design doc","flags":["DEPRECATED"],"updated":true}
```
...
`--flag` is repeatable — a note can be both `DEPRECATED` and `CONFIDENTIAL` — and `--unflag` removes
one. The set is closed and implementation-defined: a value outside `DEPRECATED`/`CONFIDENTIAL` is
rejected at write time, so a new flag is a deliberate product change, never an ad-hoc string.

Each flag carries a page of its own, stamped with the flag it describes: [[A deprecated note]] and
[[A confidential note]].
...
inline field at all — it belongs outside the body, in the note's sidecar. Code is never scanned: `std::vector` in a fenced block

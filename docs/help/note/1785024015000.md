...
native checkboxes. Nesting does not split the decision: an indented sub-list is part of the
checklist it sits in, so notation anywhere in it upgrades the whole thing, and the nesting comes
through as an indent in the table. A list item whose marker is outside the state set is not a task and stays

...
the list as a table under your cursor.) A list item whose marker is outside the state set is not a
task and stays exactly as written. The note file keeps the plain notation, so a task line remains readable

...
other web page rather than a sandboxed iframe — see *Web pages (Open Graph)* above. The iframe has a fixed
default height since a raw HTML page has no intrinsic aspect ratio, but you can set it with a `:height`
option after the embed — the same Org-style `:key value` tail [[Babel]] and note includes use:
...
page can write to your clipboard.

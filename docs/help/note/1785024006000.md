...
Mermaid, `.dot` / `.gv` for Graphviz, `.d2` for D2, `.drawio` for draw.io) becomes a **diagram** —
see [[Diagrams]] — and a
`.viewspec.json` becomes a **chart** — see [[Charts]].

## HTML pages

An HTML file is a vault asset like any other: `track asset import ./demo.html` copies it under
...
```markdown
![Demo](assets/demo.html)
```

![Demo](assets/demo.html) :height 420

A remote `http(s)://…/page.html` URL is not mounted this way: it becomes an Open Graph card like any
other web page rather than a sandboxed iframe — see *Web pages (Open Graph)* above. On a published site
that card is fetched from the reader's browser rather than the local `/api/ogp` proxy, so the reader
still connects to the remote host — one metadata request (which a cross-origin refusal degrades to a
bare link) instead of a fully rendered, scripted page. The iframe has a fixed default height since a
raw HTML page has no intrinsic aspect ratio, but you can set it with a `:height` option after the embed
— the same Org-style `:key value` tail [[Babel]] and note includes use:
...
Use `:frame none` when the embedded page should sit directly on the note without the visible frame:

```markdown
![Demo](assets/demo.html) :frame none
```

This changes only the border, rounded corners, and clipping around the iframe. The sandbox stays in
place, so the page still runs with `allow-scripts allow-popups allow-popups-to-escape-sandbox
allow-downloads allow-modals` and `allow="clipboard-write"`, and no `allow-same-origin`.

A link opened from the embedded page comes up as an ordinary tab rather than a sandboxed one, and the
frame grants the embedded page `clipboard-write` permission.

The sandbox has no `allow-forms`, and a blocked submission never dispatches the `submit` event — so an
embedded page that relies on `<form>` submit semantics silently does nothing. Drive actions from
JavaScript click handlers (`<button type="button">`) and `window.open` instead; an Enter-key shortcut
needs its own `keydown` listener.

...
final class TaskHTTP: URLProtocol, @unchecked Sendable {
    struct State { var state = "TODO"; var due = "2026-09-14"; var writes = 0; var conflict = false; var failRead = false }
    ...
    override func startLoading() {
        ...
        let value: [String: Any] = Self.state.withLock { state in
            if url.path == "/api/task" {
                ...
                if state.conflict { status = 409; return ["error": "changed"] }
            ...
            var item: [String: Any] = ["line": 1, "state": state.state, "done": done, "text": "Task"]
            ...
            case "/api/render": return ["markdown": body, "includes": []]

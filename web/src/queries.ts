...
import type { AgentRequest, DateField, NoteID, NoteMetaResponse, NoteResponse, SaveNoteMetaRequest, SaveNoteRequest } from "./types";
...
export function useAgentsQuery(enabled = !STATIC_MODE) { return useQuery({ queryKey: queryKeys.agents(), queryFn: listAgents, enabled, staleTime: 30_000 }); }
export function useAgentRequestsQuery(vault = "", enabled = !STATIC_MODE) {
  return useQuery({ queryKey: queryKeys.requests(vault), queryFn: () => listAgentRequests(vault), enabled, refetchInterval: 2000 });
}

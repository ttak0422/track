...
- `textDocument/completion`: offers titles and aliases inside an open `[[`, headings after a `[[note#` anchor (more `#` selects a deeper heading level), plus Babel fence info-string candidates, triggered on `[`, `#`, `:`, and space.

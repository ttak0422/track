...
//
// Renderers are pluggable: a Renderer maps a viewspec.Resolved onto one output format (ECharts HTML
...
type Renderer interface {
	// Name is the stable identifier used to select the renderer (e.g. "echarts").
	Name() string
	// Render produces the output document for a resolved spec.
	Render(res viewspec.Resolved) (string, error)
}

// registry holds the available renderers by name. Renderers register themselves in their package init.

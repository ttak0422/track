...
func TestStoreWritesUnderKindAssets(t *testing.T) {
	cfg := testConfig(t)

	note, err := Store(cfg, config.KindNote, "Cover Image.png", []byte("png-bytes"))
	if err != nil {
		t.Fatalf("store note asset: %v", err)
	}
	...
	if note.Path != filepath.Join(cfg.NoteDir(), "assets", "Cover-Image.png") {
	...
	journal, err := Store(cfg, config.KindJournal, "clip.png", []byte("x"))
	if err != nil {
		t.Fatalf("store journal asset: %v", err)
	}
	if journal.Path != filepath.Join(cfg.JournalDir(), "assets", "clip.png") {
		t.Fatalf("journal asset should live under journal/assets: %q", journal.Path)
	}
}

func TestStoreAvoidsOverwrite(t *testing.T) {
	cfg := testConfig(t)
	first, err := Store(cfg, config.KindNote, "a.png", []byte("1"))
	if err != nil {
		t.Fatal(err)
	}
	second, err := Store(cfg, config.KindNote, "a.png", []byte("2"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestStoreSanitizesAwkwardNames(t *testing.T) {
	...
	stored, err := Store(cfg, config.KindNote, "../etc/we(i)rd name?.png", []byte("x"))
	if err != nil {
		t.Fatal(err)
	}
	...
	if filepath.Dir(stored.Path) != filepath.Join(cfg.NoteDir(), "assets") {
...
func TestImportCopiesFile(t *testing.T) {
	...
	stored, err := Import(cfg, config.KindNote, src)
	if err != nil {

...
func mentionUpsert(t *testing.T, srv *Server, id int64, title string, aliases ...string) {
	t.Helper()
	if err := srv.store.UpsertNote(&note.Note{
		ID:   id,
		Path: srv.cfg.NotePath(id),
		Meta: note.Metadata{Title: title, Aliases: aliases},
	}); err != nil {
		t.Fatalf("upsert note %d: %v", id, err)
	}
}

// mentionLinkActions runs codeActions over the whole document and returns only the plain-text
// mention link suggestions ("Link to [[...]]").
func mentionLinkActionsForDoc(t *testing.T, srv *Server, uri, body string) []codeAction {
	t.Helper()
	srv.docs[uri] = body
	...
	actions, err := srv.codeActions(uri, rangeValue{
		...
		End:   position{Line: uint32(last), Character: uint32(len(lines[last]))},
	})
	if err != nil {
		t.Fatalf("code actions: %v", err)
	}
	...
	for _, a := range actions {
		if strings.HasPrefix(a.Title, "Link to [[") {
			out = append(out, a)
		}
	}
	return out
...
func TestCodeActionLinksPlainMention(t *testing.T) {
	srv, vault := setupServer(t)
	mentionUpsert(t, srv, 200, "test")
	uri := uriFromPath(filepath.Join(vault, "note", "201.md"))
	mentionUpsert(t, srv, 201, "Source")

	actions := mentionLinkActionsForDoc(t, srv, uri, "see test here")
	if len(actions) != 1 {
		t.Fatalf("expected one link action, got %+v", actions)
	}
	if actions[0].Title != "Link to [[test]]" {
		t.Fatalf("unexpected action title: %q", actions[0].Title)
	}
	if actions[0].Edit == nil {
		t.Fatalf("link action should carry an edit, got %+v", actions[0])
	}
	edits := actions[0].Edit.Changes[documentURI(uri)]
	if len(edits) != 1 || edits[0].NewText != "[[test]]" {
		t.Fatalf("unexpected edit: %+v", edits)
	}
	if edits[0].Range.Start.Character != 4 || edits[0].Range.End.Character != 8 {
		t.Fatalf("edit should replace only the mention range, got %+v", edits[0].Range)
	}
}

func TestCodeActionMentionOnlyWhenRangeOverlaps(t *testing.T) {
	srv, vault := setupServer(t)
	mentionUpsert(t, srv, 200, "test")
	uri := uriFromPath(filepath.Join(vault, "note", "201.md"))
	mentionUpsert(t, srv, 201, "Source")
	srv.docs[uri] = "see test here"

	// Cursor before the mention ("se|e test") yields nothing.
	before, err := srv.codeActions(uri, rangeValue{
	...
	for _, a := range before {
		if strings.HasPrefix(a.Title, "Link to [[") {
			t.Fatalf("cursor outside the mention should offer no link action, got %+v", before)
		}
	}

	// Cursor inside the mention ("te|st") offers the link.
	inside, err := srv.codeActions(uri, rangeValue{
		Start: position{Line: 0, Character: 6},
		End:   position{Line: 0, Character: 6},
	})
	if err != nil {
	...
	for _, a := range inside {
		if a.Title == "Link to [[test]]" {
			found = true
		}
	}
	if !found {
		t.Fatalf("cursor inside the mention should offer a link action, got %+v", inside)
	}
}

func TestCodeActionNoMentionForExistingLink(t *testing.T) {
	srv, vault := setupServer(t)
	mentionUpsert(t, srv, 200, "test")
	uri := uriFromPath(filepath.Join(vault, "note", "201.md"))
	mentionUpsert(t, srv, 201, "Source")

	if actions := mentionLinkActionsForDoc(t, srv, uri, "see [[test]] here"); len(actions) != 0 {
		t.Fatalf("existing link should offer no mention action, got %+v", actions)
	}
}

func TestCodeActionNoMentionInFencedCode(t *testing.T) {
	srv, vault := setupServer(t)
	mentionUpsert(t, srv, 200, "test")
	uri := uriFromPath(filepath.Join(vault, "note", "201.md"))
	mentionUpsert(t, srv, 201, "Source")

	if actions := mentionLinkActionsForDoc(t, srv, uri, "```\ntest\n```"); len(actions) != 0 {
		t.Fatalf("fenced code block should offer no mention action, got %+v", actions)
	}
}

func TestCodeActionNoMentionInInlineCode(t *testing.T) {
	srv, vault := setupServer(t)
	mentionUpsert(t, srv, 200, "test")
	uri := uriFromPath(filepath.Join(vault, "note", "201.md"))
	mentionUpsert(t, srv, 201, "Source")
	...
	if actions := mentionLinkActionsForDoc(t, srv, uri, "see `test` here"); len(actions) != 0 {
		t.Fatalf("inline code should offer no mention action, got %+v", actions)
	}
}

func TestCodeActionNoMentionInMarkdownLink(t *testing.T) {
	srv, vault := setupServer(t)
	mentionUpsert(t, srv, 200, "test")
	uri := uriFromPath(filepath.Join(vault, "note", "201.md"))
	mentionUpsert(t, srv, 201, "Source")

	if actions := mentionLinkActionsForDoc(t, srv, uri, "see [test](http://example.com) here"); len(actions) != 0 {
		t.Fatalf("markdown link should offer no mention action, got %+v", actions)
	}
}

func TestCodeActionNoMentionInURL(t *testing.T) {
	srv, vault := setupServer(t)
	mentionUpsert(t, srv, 200, "test")
	uri := uriFromPath(filepath.Join(vault, "note", "201.md"))
	mentionUpsert(t, srv, 201, "Source")

	if actions := mentionLinkActionsForDoc(t, srv, uri, "see http://example.com/test here"); len(actions) != 0 {
		t.Fatalf("URL text should offer no mention action, got %+v", actions)
	}
}

func TestCodeActionNoMentionOnH1TitleLine(t *testing.T) {
	srv, vault := setupServer(t)
	mentionUpsert(t, srv, 200, "test")
	uri := uriFromPath(filepath.Join(vault, "note", "201.md"))
	mentionUpsert(t, srv, 201, "Source")

	if actions := mentionLinkActionsForDoc(t, srv, uri, "# test"); len(actions) != 0 {
		t.Fatalf("h1 title line should offer no mention action, got %+v", actions)
	}
}

func TestCodeActionMentionAliasLinksToCanonicalTitle(t *testing.T) {
	srv, vault := setupServer(t)
	mentionUpsert(t, srv, 200, "Canonical", "alias")
	uri := uriFromPath(filepath.Join(vault, "note", "201.md"))
	mentionUpsert(t, srv, 201, "Source")

	actions := mentionLinkActionsForDoc(t, srv, uri, "see alias here")
	if len(actions) != 1 {
		t.Fatalf("expected one alias link action, got %+v", actions)
	}
	if actions[0].Title != "Link to [[Canonical]]" {
		t.Fatalf("alias should link to canonical title, got %q", actions[0].Title)
	}
	edits := actions[0].Edit.Changes[documentURI(uri)]
	if len(edits) != 1 || edits[0].NewText != "[[Canonical]]" {
		t.Fatalf("alias mention should rewrite to the canonical title, got %+v", edits)
	}
	if edits[0].Range.Start.Character != 4 || edits[0].Range.End.Character != 9 {
		t.Fatalf("edit should replace only the alias mention, got %+v", edits[0].Range)
	}
}

func TestCodeActionNoMentionForOwnTitle(t *testing.T) {
	srv, vault := setupServer(t)
	uri := uriFromPath(filepath.Join(vault, "note", "201.md"))
	mentionUpsert(t, srv, 201, "test")

	if actions := mentionLinkActionsForDoc(t, srv, uri, "see test here"); len(actions) != 0 {
		t.Fatalf("a note should not offer a mention link to itself, got %+v", actions)
	}
}

func TestCodeActionMentionPrefersLongerTitle(t *testing.T) {
	srv, vault := setupServer(t)
	mentionUpsert(t, srv, 200, "test")
	mentionUpsert(t, srv, 202, "test note")
	uri := uriFromPath(filepath.Join(vault, "note", "201.md"))
	mentionUpsert(t, srv, 201, "Source")

	actions := mentionLinkActionsForDoc(t, srv, uri, "see test note here")
	if len(actions) != 1 {
		t.Fatalf("overlapping mentions should collapse to the longest match, got %+v", actions)
	}
	if actions[0].Title != "Link to [[test note]]" {
		t.Fatalf("longer title should win, got %q", actions[0].Title)
	}
	edits := actions[0].Edit.Changes[documentURI(uri)]
	if len(edits) != 1 || edits[0].NewText != "[[test note]]" {
		t.Fatalf("unexpected edit: %+v", edits)
	}
	if edits[0].Range.Start.Character != 4 || edits[0].Range.End.Character != 13 {
		t.Fatalf("edit should span the longer mention, got %+v", edits[0].Range)
	}
}

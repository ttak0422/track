...
func viewSpecReference() string {
	...
	b.WriteString("                   SVG keeps one shared value scale (and skips y2/y3 candlestick extras)\n")
	...
	b.WriteString("                   chart, e.g. volume bars with an index line on y2); on a\n")
	b.WriteString("                   candlestick, y channels are extra series over the candles;\n")
	b.WriteString("                   under encoding.color, y[1+] are overlay series (mark required)\n")
	...
	b.WriteString("  y[].domain:      [min,max]   pin a primary quantitative value axis; values outside error\n")
	b.WriteString("  y[].opacity:     0..1   fade one series (a whole-series emphasis knob)\n")
	b.WriteString("  y[].width:       px   line/area stroke width\n")
	b.WriteString("  y[].dash:        solid | dashed | dotted   line/area stroke pattern\n")
	b.WriteString("  y[].axisName:    text   title a quantitative value axis (independent of the series)\n")
	b.WriteString("  encoding.color:  {field, type?, scale?, colors?, opacity?}   rect/treemap: cell value;\n")
	b.WriteString("                   other marks: a nominal category that splits records into one\n")
	b.WriteString("                   colored series per value; colors = {category: color} overrides,\n")
	b.WriteString("                   opacity fades every split series\n")
	...
	b.WriteString("  encoding.note:   {field}   a vault note id the datum references; the web reader\n")
	b.WriteString("                   navigates to it on click (href wins when both are set)\n")
	...
	b.WriteString("                   an event's url/note fields ride along (click-through provenance)\n")
	...
	b.WriteString("                   {yfrom, yto, axis?, label?}  shaded y-range band (value span, e.g. a target range)\n")
	...
	b.WriteString("(default 0..100), with non-overlapping primary-axis vbands as its colored zones; no x channel.\n")
	b.WriteString("\nExample:\n")
	b.WriteString(`  {
    "version": 2,
    "mark": "line",
